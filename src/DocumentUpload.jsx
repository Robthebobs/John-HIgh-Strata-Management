...
import { supabase, hasValidSupabaseCredentials, safeSupabaseCall } from '../supabaseClient';
...
const DocumentUpload = () => {
  ...
  useEffect(() => {
    ...
    if (!hasValidSupabaseCredentials) {
      setConfigError('Supabase credentials are not properly configured. Document uploads will not work.');
    }
  }, [navigate]);
  ...
  const uploadFile = async (file, index) => {
    try {
      ...
      // Create a reference to the file in Supabase Storage using safe wrapper
      ...
      // Save metadata to a table if needed, also using safe wrapper
      const { error: metadataError } = await safeSupabaseCall(() => 
        supabase
          .from('document_files')
          .insert([
            {
  ...
  const handleUpload = async (e) => {
    ...
    if (!hasValidSupabaseCredentials) {
      setUploadError('Cannot upload files: Supabase credentials are not configured');
      return;
    }
